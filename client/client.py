...
def getServerResponse(message) :
    route = requests.post("http://localhost:8080/", json= message) 
    
# Extracting path and cost from API's response and storing them for further use.
    path = route.json()['path']
    cost = route.json()['cost'] 
    
    # We have to cast "cost" as a float since SpringBoot returns it as a String.
    if float(cost) > 0 :
        # We have to split the path since Springboot returns it as a String.
        splitPath = path.split("-")
        message["start"] = splitPath [1]
    else:
